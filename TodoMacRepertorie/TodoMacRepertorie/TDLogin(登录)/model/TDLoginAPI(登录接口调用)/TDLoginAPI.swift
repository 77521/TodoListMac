...
class TDLoginAPI {
    ...
    // MARK: - 账号密码登录
    func loginWithAccount(_ account: String, password: String) async throws -> TDUserModel {
        ...
        return try await TDNetworkManager.shared.request(
            endpoint: "loginByAccount",
            parameters: parameters,
            responseType: TDUserModel.self
        )
    }
    
    // MARK: - 账号注册
    func registerAccount(_ account: String, password: String) async throws -> TDUserModel {
        let parameters: [String: Any] = [
            "userAccount": account,
            "userPassword": password
        ]
    ...
    // MARK: - 获取验证码
    func getSmsCode(phone: String) async throws {
        let parameters: [String: Any] = [
            "phoneNumber": phone
        ]

        _ = try await TDNetworkManager.shared.request(
            endpoint: "getSmsCodeByLogin",
            parameters: parameters,
    ...
    // MARK: - 手机号登录
    func loginWithPhone(_ phone: String, smsCode: String) async throws -> TDUserModel {
        let parameters: [String: Any] = [
            "phoneNumber": phone,
            "code": smsCode
        ]
