...
public class HWNetworking {
    ...
    public func request(url: String,
        ...
        // 当前版本号
        let kAppCurrentVersion = infoDictionary["CFBundleShortVersionString"]
        let kAppCurrentVersionFloat = Double(kAppCurrentVersion as! Substring)! * 100.0
        var params = parameters!
        params["versionCode"] = String(kAppCurrentVersionFloat)
        params["tdChannelCode"] = "iOS"

        // bundleId
        let kBundleId: String = Bundle.main.bundleIdentifier ?? ""
        params["packageName"] = kBundleId
        params["appName"] = "Todo清单"
        params["token"] = UserInfoDataModel.token
